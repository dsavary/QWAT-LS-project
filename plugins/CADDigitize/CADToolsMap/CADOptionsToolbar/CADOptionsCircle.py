...
class CADOptionsToolbar_Circle(CADOptionsToolbar):
    def __init__(self):
        super(CADOptionsToolbar_Circle, self).__init__()
        self.settings = QSettings()

        self.spinBox = QSpinBox(self.optionsToolBar)
        self.spinBox.setMinimum(3)
        self.spinBox.setMaximum(3600)
        segvalue = self.settings.value("/CADDigitize/circle/segments",
                                       36, type=int)
        if not segvalue:
            self.settings.setValue("/CADDigitize/circle/segments", 36)
        self.spinBox.setValue(segvalue)
        self.spinBox.setSingleStep(1)
        self.spinBoxAction = self.optionsToolBar.addWidget(self.spinBox)
        self.spinBox.setToolTip(
            tr(u"Number of quadrant segments"))
        self.spinBoxAction.setEnabled(True)

        self.spinBox.valueChanged["int"].connect(self.segmentsettingsCircle)

    def segmentsettingsCircle(self):
        self.settings.setValue("/CADDigitize/circle/segments",
                               self.spinBox.value())
