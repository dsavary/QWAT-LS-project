@echo on
SET NOM_PROJET=qwat_lausanne3d_saisie_218_valid
call "\\geodata.lausanne.ch\data\QGIS_projet\eauservice\qwat_lausanne\config_qgis\0_copy_parametres_valid.bat" %NOM_PROJET%
...
set QGIS_UDIR=%USERPROFILE%\.qgis2qwat_valid
echo %OSGEO4W_ROOT%
echo %QGIS_UDIR%
start "QGIS" /B "%OSGEO4W_ROOT%"\bin\qgis-bin.exe --configpath "%QGIS_UDIR%" --optionspath "%QGIS_UDIR%" %USERPROFILE%\QGIS\%NOM_PROJET%.qgs
