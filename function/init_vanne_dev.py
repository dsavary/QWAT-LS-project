...
def my_form_open_valve(dialog, layer, feature):
    ...
    if geom is not None and layer.isEditable():
        # declare variable formulaire
        line_function = dialog.findChild(QComboBox, "appel_fk_function")
        print line_function.currentText()
        l_fct = []
        for i in range(line_function.count()):
            l_fct.append(line_function.itemData(i))
        print l_fct
        line_closed = dialog.findChild(QCheckBox, "closed")
        print line_closed.isChecked()
