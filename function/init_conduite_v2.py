...
from qgis.gui import QgsMessageBar,QgsRelationReferenceWidget,QgsEditorWidgetWrapper,QgsRelationReferenceWidgetWrapper
...
def my_form_open(dialog, layer, feature):
    ...
    for e in enf:
        print e.children()
    '''
    relation = dialog.findChildren(QgsRelationReferenceWidget)
    #print relation
        ...
        if w.metaObject().className() == 'QgsAttributeDialog':
            ...
            for ww in w.findChildren(QComboBox):
                ...
                if ww.property('Field') and ww.property('FieldAlias'):
                    print ww
                    if ww.property('Field') == 'fk_function':
                        print "ok"

    '''
    tt = dialog.findChildren(QComboBox)
    for t in tt:
        print t.objectName()
    '''
    if geom is not None and layer.isEditable():
        ...
        def cale(checked, prec):
            #txt_prec = txts_prec[ids_prec.index(prec)]
            if checked and prec in [u"Pr\xe9cis", u"Report\xe9"]:
                line_cale_sisol.setChecked(checked)
            else:
        ...
        def preclist():
            cale(line_cale_sisol.isChecked(), line_prec.currentText())
        line_cale_sisol.stateChanged.connect(calesisol)
        line_prec.currentIndexChanged.connect(preclist)
        '''
        def precisions(checked, prec):
            list = []
            line_prec.clear()
        ...
        def yearChange():
            print line_year.text()

        line_year.textChanged.connect(yearChange)
